...
class UIManger {
    cardsContainerWrapper = document.getElementById("cards-container-wrapper");
    wrongTriesValue = document.querySelector(".wrong-tries-value");
    progressElement = document.getElementById("game-progress");
    progressBarElement = document.querySelector("#game-progress .progress-bar");
    progressNumberElement = document.querySelector("#game-progress .progress-bar-number");
    ...
    rotateCards(cards) {
        ...
        const cardElements = Array.from(this.cardsContainerWrapper.querySelectorAll("figure"));
        cards.forEach((card) => {
            card.isFlipped = true;
        });
    ...
    updateProgress(value) {
        ...
        if (this.progressElement) {
            this.progressElement.setAttribute("aria-valuenow", String(normalizedValue));
        }
    ...
    updatePauseButtonState(isPaused) {
        ...
        pauseToggleBtn.setAttribute("aria-label", isPaused ? "Resume game" : "Pause game");
    ...
    showGameResult(result, stats) {
        ...
        const normalizedProgress = Math.max(0, Math.min(100, Math.round(stats.progress)));
        ...
        `;
        document.body.appendChild(overlay);
        const replayButton = overlay.querySelector(".end-game-btn");
    ...
    flipCard(card) {
        const cardElement = this.cardsContainerWrapper?.querySelector(`figure[data-card-id="${card.id}"]`);
        if (!cardElement) {
            return;
        }
        card.flip();
